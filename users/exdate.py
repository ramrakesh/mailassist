...
for i in range(0,len(senten)) :
	...
	# 	for formats1 in formats:
	# 		matchda_fin= datetime.datetime.strptime(matchda.group(), formats1).date()
	# 	# if matchda_fin1 is not None :
	# 	# 	matchda_fin=matchda_fin1
	# 	# matchda_fin2= datetime.datetime.strptime(matchda.group(), '%Y/%m/%d').date()
	# 	# if matchda_fin2 is not None :
	# 	# 	matchda_fin=matchda_fin2
	# 	# matchda_fin3= datetime.datetime.strptime(matchda.group(), '%d-%m-%Y').date()
	# 	# if matchda_fin3 is not None :
	# 	# 	matchda_fin=matchda_fin3
	# 	# matchda_fin4= datetime.datetime.strptime(matchda.group(), '%d/%m/%Y').date()
	# 	# if matchda_fin4 is not None :
	# 	# 	matchda_fin=matchda_fin4
...
# acad_find=re.findall(r'(\bapplications?\b|\badmissions?\b|\bregistrations?\b|\ballotment\b)(\s+\S+|)',txt,re.M|re.I)
	...
	revs_find=re.findall(r'(\brescheduled?\b|\brevis[e(ed)(ion)]?\b|\bpostponed?\b|\bpreponed?\b|\bcancel[s(led)(ation)]?\b|\babandon[s(ed)]?\b|\bchange[sd]?\b|\bdelay[s(ed)]?\b|\bsuspended\b|\bconclude[sd]?\b|\bmeet(ing)?\b|\bappointment?\b)(\s+\S+|)(\s+\S+|)(\s+\S+|)',txt,re.M|re.I)
	if revs_find :

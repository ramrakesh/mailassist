...
for i in range(0,len(senten)) :
	...
	if matchda is not None:
		...
		# for fmt1 in fmt :
		# 	print fmt1
		# 	if datetime.datetime.strptime(matchda.group(),fmt1) :
		# 		matchda_fin1=datetime.datetime.strptime(matchda.group(), fmt1) 
		# 		matchda_fin=matchda_fin1.date()
		# 		print matchda_fin
		# 		if matchda_fin > today:
		...
		# if datetime.datetime.strptime(matchda.group(), '%Y-%m-%d'):
		# 	matchda_fin1=datetime.datetime.strptime(matchda.group(), '%Y-%m-%d') 
		# 	matchda_fin=matchda_fin1.date()
		
		# elif datetime.datetime.strptime(matchda.group(), '%Y/%m/%d') :
		# 	matchda_fin1=datetime.datetime.strptime(matchda.group(), '%Y/%m/%d') 
		# 	matchda_fin=matchda_fin1.date()
		
		# elif datetime.datetime.strptime(matchda.group(), '%d-%m-%Y') :
		# 	mmatchda_fin1=datetime.datetime.strptime(matchda.group(), '%d-%m-%Y') 
		# 	matchda_fin=matchda_fin1.date()
		# elif datetime.datetime.strptime(matchda.group(), '%d/%m/%Y') :
		# 	matchda_fin1=datetime.datetime.strptime(matchda.group(), '%d/%m/%Y') 
		# 	matchda_fin=matchda_fin1.date()
		# for formats1 in fmt:
		# 	matchda_fin= datetime.datetime.strptime(matchda.group(), formats1).date()
...
# acad_find=re.findall(r'(\bapplications?\b|\badmissions?\b|\bregistrations?\b|\ballotment\b)(\s+\S+|)',txt,re.M|re.I)
	...
	revs_find=re.findall(r'(\brescheduled?\b|\brevis[e(ed)(ion)]?\b|\bpostponed?\b|\bpreponed?\b|\bcancel[s(led)(ation)]?\b|\babandon[s(ed)]?\b|\bchange[sd]?\b|\bdelay[s(ed)]?\b|\bsuspended\b|\bconclude[sd]?\b|\bmeet(ing)?\b|\bappointment?\b|\bsubmi[t(ssion)]?\b|\bfunction\b|\blectures?\b)(\s+\S+|)(\s+\S+|)(\s+\S+|)',txt,re.M|re.I)
	if revs_find :
...
# # print dictio
# 		main_dict.append(dictio)
# 		values1=[id_ch,str2,rem1,flag]
# 		dictio=dict(zip(keys,values1))
# # print dictio
# 		main_dict.append(dictio1)
# 		values2=[id_ch,str3,rem1,flag]
# 		dictio2=dict(zip(keys,values2))
# # print dictio
# 		main_dict.append(dictio2)
...
# print k   
    ...
    # elif each['reminders'] =='' or each['reminders'] ==' ' or each['reminders'] == []:
    # 	k=each['id']
    # 	print k
    # 	print '3' 
    	# each['reminders']=str(today+datetime.timedelta(days=1))
